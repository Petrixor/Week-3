...
```{r}
compare <- filter(country_data,income == c('High income','Upper middle income'))
```
...
```{r}
ggplot(compare, aes(year, SP.DYN.LE00.IN, color=country,linetype=income))+geom_line()+labs(x='Year',y='Life Expectancy at birth, total(years)',title = 'Comparisons of life expectancy Between different income levels')+theme(legend.title = element_text(size =8))
```
